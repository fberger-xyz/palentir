...
import { HyperliquidInfo } from '@/interfaces'

export function HyperliquidTokens() {
    const [displayAll, setDisplayAll] = useState(false)
    const mode = `Hyperliquid tokens`
    const query = useQuery({
        queryKey: [mode],
        queryFn: async () => {
            const root = process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : APP_METADATA.SITE_URL
            const response = await fetch(`${root}/api/hyperliquid/tokens`, { cache: 'no-store' })
            if (!response.ok) throw new Error((await response.json()).error)
            const json = (await response.json()) as { data: HyperliquidInfo }
            console.log({ json })
